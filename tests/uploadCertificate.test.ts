...
test.describe('Update Personal Info', () => {
    ...
    test('Positive Test - Upload certificate - Decline request', async () => {
        ...
        //step 12: Assertion : Verify Notification title confirming that changes has been declined
        await test.step('Validate notification message title', async () => {
            const notifMsg = await page.locator('div').filter({ hasText: ' Changes in Certifications  for NelTest Emp were declined' }).nth(2);
            await expect(notifMsg).toBeVisible();
        });
        ...
        // //step 16 : verify notification that request has been declined

        await test.step('Check notification', async () => {
            await manPage.checkNotif();
        });

        //step 17: Assertion : Verify Notification title confirming that changes has been approved
        await test.step('Validate notification message title', async () => {
            const notifMsg = await page.locator('div').filter({ hasText: ' Changes in Certifications  for NelTest Emp were declined' }).nth(2);
            await expect(notifMsg).toBeVisible();
        });
